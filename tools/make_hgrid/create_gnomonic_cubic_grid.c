/*******************************************************************************
  ...
  11/23/2020  -- Sanity check against new FV3 dycore codebase.
                 Formatting changes.
                 Kyle Ahern, AOML/HRD
*******************************************************************************/
        ...
        int halo)
{
  ...
  /* make sure the first 6 tiles have the same grid size and
     ...
     ntiles is a constant always equal to 6.
     ntiles2 is variable, and includes the 6 global tiles plus any nests
  */
  ...
  for(n=0; n<ntiles; n++) {
    if(nlon[n] != nlat[n])
      mpp_error("create_gnomonic_cubic_grid: the grid size in x and y-direction "
                "should be the same for the 6 tiles of cubic sphere grid");
    if( nlon[n]%2 )
      mpp_error("create_gnomonic_cubic_grid: supergrid size in x-direction should be divided by 2");
    if( nlat[n]%2 )
      mpp_error("create_gnomonic_cubic_grid: supergrid size in y-direction should be divided by 2");
  }
  for(n=1; n<ntiles; n++) {
    if(nlon[n] != nlon[0])
      mpp_error("create_gnomonic_cubic_grid: all six tiles should have same size");
  }
  ...
  */
  ...
  /* Cycle through all of the tiles; global and nests,
     adding enough points based on the dimensions
     The 6 cubed-sphere tiles are square thus, nil=njl,
     but the nests can be rectangular
  */
  npts = 0;
  npts_supergrid = 0;
  npts_supergrid_m = 0;
  ...
  for (n=0; n<ntiles2; n++) {
    ...
    if (verbose)
      printf("[INFO] IDX OFFSET n: %d tile_offset[n]: %d tile_offset_supergrid[n]: %d tile_offset_supergrid_m[n]: %d tile_offset_area[n]: %d \n",
  ...
  for(n=0; n<ntiles*nip*nip; n++) {
    ...
    // NOTE: Original multi-nest implementation uses xc[n] -= M_PI/18.0; [Ahern]
    if( do_schmidt == 0 && shift_fac > EPSLN4) xc[n] -= M_PI/shift_fac;
  ...
  if ( do_schmidt ) {
    for(n=0; n<ntiles; n++) {
      if (verbose) printf("[INFO] Calling direct_transform for tile %d\n", n);
  ...
  /* else if( nest_grid ) {
    ...
    for (nn=0; nn < num_nest_grids; nn++) {
      ...
      /* Setup aligned nest -- final two arguments are
         memory locations for data to be returned.
    ...
    if (verbose) printf("[INFO] Completed processing setup_aligned_nest for nest(s)\n");
  }
  ...
  for(n=0; n<ntiles2; n++) {
    ...
    if (verbose) {
      printf("[INFO] INDEX fill x and y from C-cell.  n=%d n*nxp*nxp=%d tile_offset[n]: %d tile_offset_supergrid[n]: %d njl[n]: %d nil[n]: %d\n",
             n, n*nxp*nxp, tile_offset[n], tile_offset_supergrid[n], njl[n], nil[n]);
      printf("[INFO] START fill x and y from C-cell.  n=%d tile_offset_supergrid[n]: %d \n",
    ...
    for(j=0; j<njl[n]; j++) for(i=0; i<nil[n]; i++) {
      ...
      n2 = j*nil[n]+i;   /* why does this not have a tile_offset?? [WDR] */
      x[n1]=xtmp[n2];
      y[n1]=ytmp[n2];
    ...
    for(j=0; j<njl[n]; j++) for(i=0; i<=nil[n]; i++) {
      ...
      n2 = j*(nil[n]+1)+i;   /* why does this not have a tile_offset?? [WDR] */
      x[n1]=xtmp[n2];
      y[n1]=ytmp[n2];
    ...
    for(j=0; j<=njl[n]; j++) for(i=0; i<nil[n]; i++) {
      ...
      n2 = j*nil[n]+i;   /* WDR why does this not have a tile_offset?? [WDR] */
      x[n1]=xtmp[n2];
      y[n1]=ytmp[n2];
    ...
    if (verbose) {
      printf("[INFO] INDEX tile: %d min_n1: %d max_n1: %d max_n1 - min_n1: %d sqrt(max_n1 - min_n1 + 1): %f\n",
  ...
  for(n=0; n<ntiles2; n++) {
    if (verbose) {
      printf("[INFO] M2a Calculating dy for tile n: %d ntiles: %d ntiles2: %d\n",
             n, ntiles, ntiles2);
    }

    if( stretched_grid || (n >= 6) ) {
      if (verbose) {
        printf("[INFO] M2b Calculating dy for tile n: %d ntiles: %d ntiles2: %d\n",
               n, ntiles, ntiles2);
      }
      ...
      if (verbose) {
        printf("[INFO] M2c Calculating dy for tile n: %d ntiles: %d ntiles2: %d\n",
               n, ntiles, ntiles2);
      }
      for(j=0; j<nyp; j++) {
        for(i=0; i<nx; i++) {
          dy[tile_offset_supergrid_m[n] + i*nxp+j] = dx[tile_offset_supergrid_m[n] + j*nx+i];
        }
      }
    }
  } /* for(n=0; n<ntiles2; n++) */
  ...
  for (nn=0; nn < num_nest_grids; nn++) {
    if (verbose) {
      printf("[INFO] call calc_cell_area for nest nn=%d tile n=%d\n", nn, ntiles+nn);
      printf("[INFO] Q1\n");
    }
    calc_cell_area(nx_nest_arr[nn], ny_nest_arr[nn],
                   x + tile_offset_supergrid[ntiles+nn], y + tile_offset_supergrid[ntiles+nn],
  ...
  if (verbose) {
    printf("[INFO] Convert radians to degrees: npts = %d npts_supergrid: %d\n",
           npts, npts_supergrid);
  }
		      ...
		      int n, double *lon, double *lat)
{
  ...
  for(j=j1; j<=j2; j++) for(i=i1; i<=i2; i++) {
    ...
    if ( (1.-fabs(sin_o)) < EPSLN7 ) {    /* poles */
      ...
      if ( lon[l] < 0. ) lon[l] +=two_pi;
      else if( lon[l] >= two_pi ) lon[l] -=two_pi;
    }
  }
}; /* direct_transform */
