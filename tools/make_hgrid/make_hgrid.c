/*
  ...
  11/23/2020 -- Updated usage statement, sanity check against
...
int main(int argc, char* argv[])
{
  ...
  int    nxbnds0 = 0, nybnds0 = 0;
  int    nxbnds1 = 0, nybnds1 = 0;
  int    nxbnds2 = 0, nybnds2 = 0;
  int    nxbnds3 = 0, nybnds3 = 0;

  double f_plane_latitude = 100;
  double lat_join   = 65.0;
  ...
  /* Variables for multiple nesting */
  int nest_grids    = 0;
  int num_nest_args = 0;
  int nn = 0;
  ...
  static struct option long_options[] = {
    ...
    {"nest_grid",       no_argument,       NULL, 'L'}, /* Obsolete (bkward compatible) */
  ...
  while ((c = getopt_long(argc, argv, "", long_options, &option_index)) != -1) {
    ...
    case 'K':
      ...
      break;
    case 'L': /* Single-nest option backwards compatibility */
      nest_grids = 1;
      break;
    case 'v':
      verbose = 1;
  ...
  if( my_grid_type == TRIPOLAR_GRID ) {
    ...
    */
    ...
    for(n=0; n<ntiles; n++) {
      if( strstr(my_grid_file[n],".nc") ) {
        ...
        if( mpp_dim_exist(fid, "grid_xt") ) {
      	...
      	mpp_close(fid);
      } /* end if ( strstr(my_grid_file[n],".nc") ) */
      else {
	      if( nxbnds2 != ntiles || nybnds2 != ntiles )
          mpp_error("make_hgrid: grid type is 'from_file', number entry entered "
						        "through --nlon and --nlat should be equal to number of files "
							      "specified through --my_grid_file");
      }
    } /* end for n=0 to n<ntiles */
    /* for simplify purpose, currently we assume all the tile have the same grid size */
    for(n=1; n<ntiles; n++) {
      if( nlon[n] != nlon[0] || nlat[n] != nlat[0])
        mpp_error("make_hgrid: grid_type is from_file, all the tiles should "
    ...
    if(nxbnds1 != 2 || nybnds1 != 2 )
      mpp_error("make_hgrid: grid type is 'simple_cartesian_grid', number entry entered "
                "through --xbnds and --ybnds should be 2");
    if(nxbnds2 != 1 || nybnds2 != 1 )
      mpp_error("make_hgrid: grid type is 'simple_cartesian_grid', number entry entered "
                "through --nlon and --nlat should be 1");
    if(simple_dx == 0 || simple_dy == 0) 
      mpp_error("make_hgrid: grid_type is 'simple_cartesian_grid', "
                "both simple_dx and simple_dy both should be specified");
  }
  else if( my_grid_type == SPECTRAL_GRID ) {
    if(nxbnds2 != 1 || nybnds2 != 1 )
      mpp_error("make_hgrid: grid type is 'spectral_grid', number entry entered "
                "through --nlon and --nlat should be 1");
  }
    ...
    if(nxbnds2 != 1 )
      mpp_error("make_hgrid: grid type is 'conformal_cubic_grid', number entry entered "
    ...
    for (n=0; n < nest_grids; n++) {
      if(refine_ratio[n] <= 0) mpp_error("make_hgrid: --refine_ratio must be set and positive when --nest_grids is set");
      if(parent_tile[n] <= 0)  mpp_error("make_hgrid: --parent_tile must be set and positive when --nest_grids is set");
      if(istart_nest[n] <= 0)  mpp_error("make_hgrid: --istart_nest must be set and positive when --nest_grids is set");
      if(iend_nest[n] <= 0)    mpp_error("make_hgrid: --iend_nest must be set and positive when --nest_grids is set");
      if(jstart_nest[n] <= 0)  mpp_error("make_hgrid: --jstart_nest must be set and positive when --nest_grids is set");
      if(jend_nest[n] <= 0)    mpp_error("make_hgrid: --jend_nest must be set and positive when --nest_grids is set");
      if(halo <= 0)            mpp_error("make_hgrid: --halo must be set and positive when --nest_grids is set");
    ...
    if(nxbnds2 != 1)
      mpp_error("make_hgrid: grid type is 'gnomonic_cubic_grid', number entry entered "
                "through --nlon should be 1");
  }
  ...
  /* write out data */
  {
    ...
    for(n=0 ; n< ntiles; n++) {
      ...
      if(ntiles>1) sprintf(outfile, "%s.tile%d.nc", gridname, n+1);
      else sprintf(outfile, "%s.nc", gridname);

      if (verbose) printf("Writing out %s.\n", outfile);
      ...
      if(out_halo == 0) {
        if (verbose) {
          printf("[INFO] START NC XARRAY write out_halo=0 tile number = n: %d offset = pos_c: %d\n",
                 n, pos_c);
          printf("[INFO] XARRAY: n: %d x[0]: %f x[1]: %f x[2]: %f x[3]: %f x[4]: %f x[5]: %f x[10]: %f\n",
      ...
      if (verbose) printf("About to close %s\n", outfile);

      mpp_close(fid);

      /* Advance the pointers to the next tile.
         Use the size of a full panel, not the nest,
         because code in create_gnomonic_cubic_grid
         uses ntile*size */
      ...
      if (verbose) printf("[INFO] INDEX Before increment n: %d pos_c %d nxp %d nyp %d nxp*nyp %d\n",
                          n, pos_c, nxp, nyp, nxp*nyp);
      pos_c += nxp*nyp;
      if (verbose) printf("[INFO] INDEX After increment n: %d pos_c %d.\n", n, pos_c);
      pos_e += nxp*ny;
      pos_n += nx*nyp;
      pos_t += nx*ny;
    }
